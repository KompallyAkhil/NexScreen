...
export function ScoreGauge({ score, band }: { score: number; band: string }) {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center p-8 rounded-3xl glass overflow-hidden shadow-2xl">
      ...
      <div
        className={cn(
          "absolute inset-0 opacity-20 blur-3xl -z-10 transition-colors duration-1000",
          score >= 75
            ? "bg-emerald-500"
            : score >= 50
              ? "bg-amber-500"
      ...
      <div className="relative w-56 h-56 flex items-center justify-center">
        <svg className="w-full h-full -rotate-90">
          ...
          <motion.circle
            ...
            className={cn(
              "fill-none drop-shadow-[0_0_15px_rgba(255,255,255,0.3)]",
              strokeColor,
            )}
            strokeWidth="16"
            strokeDasharray="628.3"
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center mt-6">
          ...
          <span className="text-zinc-400 text-xs font-bold uppercase tracking-[0.2em] mt-1">
            Match Score
          </span>
        </div>
      </div>
      ...
      <motion.div
        ...
        transition={{ delay: 1 }}
        className={cn(
          "mt-6 px-5 py-2 rounded-full text-xs font-bold uppercase tracking-widest border shadow-lg backdrop-blur-md",
          score >= 75
