...
export function InsightCard({ title, items, type, delay = 0 }: { title: string; items: string[]; type: 'strength' | 'concern' | 'info', delay?: number }) {
  const configs = {
    strength: { icon: <TrendingUp className="text-emerald-400" size={22} />, gradient: "from-emerald-500/10" },
    concern: { icon: <TrendingDown className="text-rose-400" size={22} />, gradient: "from-rose-500/10" },
    info: { icon: <Info className="text-indigo-400" size={22} />, gradient: "from-indigo-500/10" }
  };
  ...
  return (
    ...
    >
      ...
      <ul className="space-y-4 relative z-10">
        {items.map((item, i) => (
          <li key={i} className="flex gap-4 text-sm text-zinc-300/90 leading-relaxed items-start">
